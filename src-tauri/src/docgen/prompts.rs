...
This folder contains everything you need to build [one-sentence description] using Claude Code (or any AI coding assistant). The planning is done — the documents guide execution.
...
pub const START_HERE_PROMPT: &str = r##"Generate START_HERE.md — the bridge document for users who are new to Claude Code or non-technical.
...
### 2. Copy CLAUDE.md
Copy the `CLAUDE.md` file from this planning folder into your new project folder.
