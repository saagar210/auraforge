...
## Snapshot
- Timestamp (UTC): `2026-02-22T11:55:46Z`
- Status: `Blocked`
...
| Signed release dispatch works (`require_signed=true`) | Fail | Run `22276565984` failed in step `Enforce signed mode when required` with message: missing `APPLE_*` secrets | AuraForge PM + Eng |
...
| `gh run view 22276565984 -R saagar210/auraforge --log-failed` | Fail (expected) | Signed-required run fails early with explicit missing-secrets message |
